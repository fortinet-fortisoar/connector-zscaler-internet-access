...
logger = get_logger('zscaler-internet-access')
...
class ZscalerAuth:
    ...
    def make_api_call(self, config, connector_info, token_url='', method="GET", endpoint="", params=None, headers=None,
                      data=None,
                      json_data=None,
                      verify_ssl=False):
        try:
            headers = headers if headers else {"Authorization": self.validate_token(config, connector_info)}
            endpoint = self.server_url + endpoint if not token_url else token_url
